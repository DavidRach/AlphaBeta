...
ProcessingFunction <- function(x){
    ...
    Data <- Data |> mutate(Designation = case_when(
        ...
        str_replace("^([^ ]+) (.+)$", "\\2 \\1"),
        str_detect(filename, "Reference Group-DR_.* \\(Beads\\)") ~ 
        str_extract(filename, "(?<=Reference Group-DR_)[^\\(]+(?= \\(Beads\\))") |>
        str_replace("^([^ ]+) (.+)$", "\\2 \\1"),
        TRUE ~ "Sample")) 
...
Data <- Data |> mutate(BiosampleID="ND006", SubjectID="ND006")

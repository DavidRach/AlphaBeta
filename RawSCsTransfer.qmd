...
```{r}
#CSV <- paste("https://raw.githubusercontent.com", "DavidRach", "AlphaBeta", "main", "data", "ProcessedExperiments_SCs.csv", sep = "/") 

#Data <- read.csv(CSV, check.names=FALSE)
Data <- read.csv("data/ProcessedExperiments_SCs.csv")
...
```{r}
#| eval: FALSE
UpdatedData <- purrr::map(.x=TheseFolders, .f=ExperimentParsed, parsed=AlreadyProcessed, parentFolder=OS, Type="Raw", Samples="singlecolors")
...
write.csv(NewlyProcessed, "data/ProcessedExperiments_SCs.csv", row.names=FALSE)
```
